...
# ---------------------------------------------------------------------------
import kinextract as sf  # type: ignore
...
def _silence_log(msg: str) -> None:
    """Drop-in replacement for log() that does nothing."""
    pass


def _n_params(st) -> int:
    """Total number of free parameters in the MAP vector."""
    n = st.nl + st.nt
    if st.icoff == 1:
        n += 2
    elif st.icoff == 2:
...
def compute_hessian_from_grad_fd(
    ...
    """
    Compute Hessian from central finite differences of an analytic gradient.

    This needs 2*n gradient evaluations versus O(n^2) objective evaluations,
    and is much faster when a JAX gradient is available.
    """
    a = np.asarray(a_map, float).copy()
    n = len(a)
    H = np.zeros((n, n), float)
...
def compute_losvd_hat_matrix(st, a_map: np.ndarray) -> np.ndarray:
    ...
    *linear* in the LOSVD vector b (through the convolution).  In this
    ...
    """
    from kinextract import evaluate_model_gp, _compute_smoothness
...
def _refit_one_bootstrap(
    ...
    try:
        ...
        if st.fit_als_continuum:
            ...
            sf.init_als_continuum(st, cfg_frozen, templates=getattr(st, 't', None))
        ...
        res = sf.fit_state_map_with_optional_clean(
        ...
        gp, b, w, coff, coff2, A = sf.evaluate_model_gp(a_boot, st)

        gh = sf.fit_losvd_gauss_hermite(st.xl, b, fit_h3h4=True)
        gh_ho = sf.fit_losvd_gauss_hermite_higher(st.xl, b, max_order=6)
...
class LOSVDErrorEstimator:
    """
    Unified interface for LOSVD error estimation.
    ...
    def __init__(self, fit: dict, cfg):
        ...
        _, xlb, xub = sf.build_initial_guess_nonparam(self.st, cfg.coff, cfg.coff2)
    ...
    def laplace_covariance(
        ...
        dict with keys:
            ...
            is_pd    : bool — True if H was PD before regularization
        ...
        H = None
        if hasattr(sf, "_build_jax_objective_value_and_grad"):
            try:
                if getattr(self.cfg, "jax_enable_x64", True) and getattr(sf, "jax", None) is not None:
                    sf.jax.config.update("jax_enable_x64", True)
                vg = sf._build_jax_objective_value_and_grad(self.st)
                if vg is not None:
        ...
        if H is None:
            ...
            def obj(a):
                return sf.objective_map(a, self.st)

            H = compute_hessian_fd(obj, self.a_map, rel_step=rel_step)

        # objective_map() returns chi2 + penalty terms.  The Laplace
        # approximation uses the Hessian of the negative log posterior,
        # which is 0.5 * (chi2 + penalty) under the usual Gaussian-noise
        # convention.  Without this factor the covariance is too small by
        # roughly a factor of 2.
        H = 0.5 * H

        C, H_reg, is_pd = laplace_posterior_covariance(
            H, regularize_nugget=regularize_nugget,
        )

        nl, nt = self.st.nl, self.st.nt
        C_b = C[:nl, :nl]
        C_w = C[nl:nl + nt, nl:nl + nt]
        b_err = np.sqrt(np.maximum(np.diag(C_b), 0.0))
        w_err = np.sqrt(np.maximum(np.diag(C_w), 0.0))
    ...
    def bias_correction(self) -> dict:
        """
        Estimate and subtract the regularization bias from b_map.

        Returns
        -------
        dict with keys:
            H_b        : (nl, nl) influence matrix
            b_corr     : (nl,)    bias-corrected LOSVD
            bias       : (nl,)    estimated regularization bias
            eff_dof    : float    effective degrees of freedom = tr(H_b)
        """
    ...
    def _gh_errors_delta_method(self, C_b: np.ndarray) -> dict:
        ...
        def gh_vec(b):
            gh = sf.fit_losvd_gauss_hermite(xl, b, fit_h3h4=True)
            return np.array([
    ...
    def summarize(
        ...
        gh_map = sf.fit_losvd_gauss_hermite(self.xl, self.b_map, fit_h3h4=True)
        gh_ho_map = sf.fit_losvd_gauss_hermite_higher(self.xl, self.b_map, max_order=6)
        ...
        if bootstrap_result is not None:
            ...
            gh_center = sf.fit_losvd_gauss_hermite(self.xl, b_center, fit_h3h4=True)
    ...
    def _print_summary(self, summary: dict) -> None:
        ...
        print(f"  Gauss-Hermite moments (MAP, consistent with pallmc.f):")
        ...
        print(f"  LOSVD moments (for reference):")
        print(
    ...
    def plot_losvd_with_errors(
        ...
        """
        Plot the LOSVD with error bands.

        Shows:
          - MAP LOSVD (solid blue)
          - Bootstrap/Laplace 1 sigma band (shaded)
          - Bias-corrected LOSVD (dashed orange) if available
          - Gauss-Hermite fit with annotated moments
        """
        ...
        with mpl.rc_context({"text.usetex": False}):
            ...
            v_meas, sigma_meas = _losvd_moments(xl, b_center)
            moments = summary.get("moments_center_recommended", {"v": v_meas, "sigma": sigma_meas})
            moments_err = summary.get("moments_err_recommended", {})
    ...
    def plot_bootstrap_distribution(
        ...
        """
        Diagnostic: show the full distribution of bootstrap b and GH moments.
        """
        import matplotlib.pyplot as plt
        ...
        # sf is imported at module level from kinextract
        gh_map = sf.fit_losvd_gauss_hermite(xl, self.b_map, fit_h3h4=True)
        gh_map_vals = {
    ...
    def plot_spectrum_with_errors(
        ...
        """
        Plot the final best-fit spectrum with observational error bands.

        Mirrors the kinextract-style data/model/residual diagnostic, but keeps
        the uncertainty visible so the fit quality is easy to judge.
        """
        import matplotlib as mpl
        import matplotlib.pyplot as plt
        # sf is imported at module level from kinextract
        ...
        try:
            if "bootstrap" in summary and summary["bootstrap"] is not None and "b_samples" in summary["bootstrap"]:
                ...
                for k in idx:
                    ...
                    gp_s, *_ = sf.evaluate_model_gp(a_s, self.st)
                    gps.append(gp_s)
                gps = np.vstack(gps)
...
def plot_losvd_with_errors(fit: dict, cfg, summary: dict, show: bool = True):
    """Convenience wrapper around LOSVDErrorEstimator.plot_losvd_with_errors."""
    est = LOSVDErrorEstimator(fit, cfg)
...
def plot_spectrum_with_errors(fit: dict, cfg, summary: dict, show: bool = True):
    """Convenience wrapper around LOSVDErrorEstimator.plot_spectrum_with_errors."""
    est = LOSVDErrorEstimator(fit, cfg)
