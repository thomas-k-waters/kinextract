"""Sweep n_losvd_bins (29 vs 89) across the full sigma range to decide the new default."""
...
if __name__ == "__main__":
    ...
    results = []
    t0 = time.time()
    print(f"{'sigma':>6s} {'seed':>5s} {'nbins':>6s} {'V':>8s} {'sigma_rec':>9s} {'chi2_red':>9s} {'time':>7s}")
    for sigma in sigmas:
        for seed in seeds:
            ...
            for nbins in [29, 89]:
                ...
                results.append(row)
                print(f"{sigma:6.0f} {seed:5d} {nbins:6d} {r['v']:8.2f} {r['sigma_rec']:9.2f} {r['chi2_red']:9.3f} {r['time']:7.1f}s")

    print(f"\nTotal: {time.time()-t0:.0f}s")
    print(f"\n{'sigma':>6s} {'nbins':>6s} {'Vbias':>8s} {'Vstd':>7s} {'sigbias':>8s} {'sigstd':>7s} {'avg_time':>9s}")
    for sigma in sigmas:
        for nbins in [29, 89]:
            rows = [r for r in results if r["sigma"] == sigma and r["nbins"] == nbins]
            vb = np.array([r["v"] for r in rows]) - true_v
            sb = np.array([r["sigma_rec"] for r in rows]) - sigma
            tt = np.array([r["time"] for r in rows])
            print(f"{sigma:6.0f} {nbins:6d} {vb.mean():+8.2f} {vb.std():7.2f} {sb.mean():+8.2f} {sb.std():7.2f} {tt.mean():9.1f}")

    with open("dev_notes/stress_test/results_nbins_sweep.json", "w") as f:
